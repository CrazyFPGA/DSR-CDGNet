...
>
> 本工作由本人与 **巴西国家石油公司（Petrobras）** 的 **Ricardo Emanuel Vaz Vargas** 博士合作完成。
>
...
为此，本项目提出了 **DSR-CDGNet**（Hierarchical Spatio-temporal Reasoning with Dynamic Inference Gating Network，动态时空推理与置信驱动门控模型），主要贡献包括：

1. **节点完整性维护策略**：权衡测量数据质量与拓扑完整性。
2. **数据预处理机制**：从原始信号中提取包含"状态"与"趋势"的特征序列，实现多源测量数据的降维与增强。
3. **物理图拓扑结构**：首次在油井故障检测问题中引入基于传感器布局的物理图拓扑。
4. **中心性引导图卷积单元（CGC）**：建模传感器间的空间关系。
5. **双流注意力机制**：在时间维度上通过迭代精炼强化系统级时序测量特征的表达。
6. **置信驱动的动态门控机制（Dynamic ITR）**：在保障检测置信度的同时实现高效推理。
...
| **`model.py`** | 定义 **DSR-CDGNet** 主模型以及 `AdaptiveGraphLearner`（GRU 驱动的自适应图学习器）、`CGCConv`（中心性引导图卷积）、迭代推理块（Dynamic ITR / Dynamic ITR_CE / Single ITR）等组件。同时内置多种 GCN 变体（GCN / GAT / SAGE / APPNP / GIN / ChebConv）用于消融对比，以及 GRU / MLP / LSTM / Transformer 等时序骨干用于对照实验。 |
...
> ⚠️ 数据本身不包含在本仓库中，请从 [3W Dataset 官方仓库](https://github.com/petrobras/3W) 下载，并将路径配置到 `config.yaml` 的 `ROOT_DATA_PATH`。
...
从 [3W 官方仓库](https://github.com/petrobras/3W) 获取数据，将解压后的数据路径填入 `config.yaml`：
...
其中 \(\text{PR}(j)\) 为节点 \(j\) 的 PageRank 中心性分数，\(e_{ij}\) 为边权重。

随后通过 \(N=5\) 次迭代精炼块与置信门控：

\[
\mathcal{L} = \sum_{k=1}^{N} C_k \cdot \mathcal{L}_{\text{CE}}^{(k)} + \lambda \cdot \mathcal{L}_{\text{penalty}}
\]

其中 \(C_k\) 为第 \(k\) 次迭代的归一化置信度门控，\(\lambda\) 按训练进度在 \([\lambda_{\text{initial}}, \lambda_{\text{final}}]\) 区间阶梯式调度。
