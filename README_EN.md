...
>
> This work received the guidance of **Dr. Ricardo Emanuel Vaz Vargas** from **Petrobras (Petróleo Brasileiro S.A.)**, Brazil, who is also a co-author of this paper.
...
To address these issues, we propose **DSR-CDGNet** (Dynamic spatio-temporal reasoning and confidence-driven gating model), whose main contributions are:
...
4. **Centrality-Guided Graph Convolution** — models the spatial relationships among sensors.
5. **Dual-stream attention with iterative refinement** — strengthens system-level temporal features through iterative reasoning.
6. **Confidence-driven dynamic gating** — ensures detection confidence while enabling efficient inference.
...
| Inference FLOPs Reduction | 20%-25% | 20%-25% |
...
| **`model.py`** | Defines the **DSR-CDGNet** model and its components: `AdaptiveGraphLearner` (GRU-driven adaptive graph learner), `CGCConv` (centrality-guided graph convolution), and the iterative-reasoning blocks (Dynamic ITR / Dynamic ITR_CE / Single ITR). Also ships with ablation variants (GCN / GAT / APPNP / GIN / ChebConv) and temporal baselines (GRU / MLP / LSTM / Transformer). |
...
Point `config.yaml` to actual data path:
